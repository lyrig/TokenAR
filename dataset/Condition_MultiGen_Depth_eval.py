...
# MultiGen dataset
class MultiGen_Depth_Eval_Dataset(Dataset):
    def __init__(self,
                 args,
                 dataset_path,
                 llm_tokenizer,
    ...
    def __getitem__(self, index):
        ...
        edit_txt = 'Given the depth image, generate an image following the description: '+data['text']
        ...
        save_input_img.save(f"{self.args.gpt_ckpt[:-3]}/depth/input/depth_eval_{index:05d}_input.png")
        save_edited_img = (edited_img.permute(1,2,0)+1)/2 * 255.
        save_edited_img = Image.fromarray(np.array(save_edited_img).astype(np.uint8))
        save_edited_img.save(f"{self.args.gpt_ckpt[:-3]}/depth/edit/depth_eval_{index:05d}_edit.png")
        # Create the content to be saved
        content = f"Edited Text:\n{edit_txt}"
        # Save the content to the file
        with open(f'{self.args.gpt_ckpt[:-3]}/depth/text/depth_eval_{index:05d}_txt.txt', "w") as file:
            file.write(content)
