...
# Condition segmentation dataset
class Condition_Segmentation_Eval_Dataset(Dataset):
    def __init__(self,
                 args,
                 dataset_path,
                 llm_tokenizer,
        ...
        self.args = args
        self.dataset_path = load_from_disk(dataset_path+'/Captioned_COCOStuff_eval_HF')
    ...
    def __getitem__(self, index):
        ...
        _input_img = np.array(input_img)
        _label_img = data['panoptic_seg_map']
        _label_img = np.array(_label_img).astype('uint8')

        # input_txt = data['original_prompt']
        # edited_txt = data['edited_prompt']
        if 'detailed_prompt' in data.keys():
            edit_txt = 'Given the segmentation mask, generate an image following the description: '+data['detailed_prompt']
        else:
            edit_txt = 'Given the segmentation mask, generate an image following the description: '+data['prompt']
        ...
        save_input_img.save(f"{self.args.gpt_ckpt[:-3]}/conditionsegmentation/input/condition_seg_{index:05d}_input.png")
        save_edited_img = (edited_img.permute(1,2,0)+1)/2 * 255.
        save_edited_img = Image.fromarray(np.array(save_edited_img).astype(np.uint8))
        save_edited_img.save(f"{self.args.gpt_ckpt[:-3]}/conditionsegmentation/edit/condition_seg_{index:05d}_edit.png")
        _label_img = Image.fromarray(_label_img)
        _label_img.resize((512, 512), Image.Resampling.NEAREST).save(f"{self.args.gpt_ckpt[:-3]}/conditionsegmentation/label/{index:05d}.png")
        # Create the content to be saved
        content = f"Edited Text:\n{edit_txt}"
        # Save the content to the file
        with open(f'{self.args.gpt_ckpt[:-3]}/conditionsegmentation/text/condition_seg_{index:05d}_txt.txt', "w") as file:
            file.write(content)

        return {
                'index': index,
                'dataset': 'condition_seg',
                'mode': 1,
                'input_ids': input_ids,
