...
def setup(rank, world_size):
    # This is a basic setup using the 'gloo' backend, but 'nccl' is preferred for GPUs
...
def build_save_paths(batch, args, ckpt_num):
    save_paths = []
    for sample_path, dataset_name in zip(batch["path"], batch["dataset"]):
        if args.max_ref_num <= 1:
            filename = f"{dataset_name}_sample_txt_{args.cfg_scale}_multi_cond_test_{ckpt_num}.png"
        else:
            if args.additional_info is None:
                filename = f"sample_txt_{args.cfg_scale}_multi_cond_test_{ckpt_num}.png"
            else:
                filename = f"{args.additional_info}_sample_txt_{args.cfg_scale}_multi_cond_test_{ckpt_num}.png"
        save_paths.append(os.path.join(sample_path, filename))
    return save_paths


def save_batched_images(samples, save_paths):
    for sample, save_path in zip(samples, save_paths):
...
def main(args):
    ...
    dataset = MultiCond_IP_Plus_Dataset(args, dataset_path=data_path, llm_tokenizer=t5_model.tokenizer, mode='val', need_bg=need_bg, max_cond=args.max_ref_num, world_size=world_size, rank=rank)
    ...
    for idx, batch in tqdm(enumerate(loader), total=len(loader), desc="Generating Processing:"):
        if args.max_ref_num <= 1:
            ...
            input_img = batch['input_img'][0].to(device, non_blocking=True)
            # New Feature
            if need_bg:
            ...
            save_paths = build_save_paths(batch, args, ckpt_num)
            ...
            save_batched_images(samples, save_paths)
        else:
            ...
            if args.special_mask:
                ...
                total_time.append(end_generate_time - start_generate_time)
            ...
            if args.concat_target and args.instruct_token_mode != "special":
                target_sample = index_sample[:, -latent_size**2:]
                samples = vq_model.decode_code(target_sample, qzshape)
            else:
                samples = vq_model.decode_code(index_sample, qzshape)
            for sample_idx, sample in enumerate(samples):
                writer.add_image(
                    f"Image_{idx * args.batch_size + sample_idx}",
                    sample,
                    global_step=0,
                ) # type: ignore
            save_batched_images(samples, save_paths)
    ...
    ckpt_num = os.path.basename(args.gpt_ckpt).split(".")[0]
    time_file = os.path.join(data_path, f"{args.additional_info}_sample_txt_{args.cfg_scale}_multi_cond_test_{ckpt_num}_time.txt")
    if args.rank==0:
        with open(time_file, "w+") as f:
            f.write(f"Gerneated Time: {mean_time}")
            print(f"Time Saving: {time_file}")
# @torch.no_grad()
...
if __name__ == "__main__":
    ...
    # New Feature
    parser.add_argument("--add_ref_embed", action="store_true", default=False, help="Add Some Learnable Reference Embedding")
    ...
    parser.add_argument("--world-size", type=int, default=8, help="The Instruction Token Number in model.")
    parser.add_argument("--rank", type=int, default=0, help="The Instruction Token Number in model.")
