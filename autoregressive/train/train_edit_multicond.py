...
def main(args):
    ...
    if rank == 0:
        ...
        json_path = f"{checkpoint_dir}/config.json"
        import json
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in loader:
            ...
            if args.special_mask:
                ...
                mask_len = (len(input_img_list) + 1) * block_len + args.cls_token_num + edited_img_indices.shape[1] - 1
                mask = torch.ones(size=(mask_len, mask_len), dtype=torch.bool).tril(0) # Initial a full mask.
                mask[:input_img_indices.shape[1]] = False
                for ref_index in range((len(input_img_list) + 1)):
                    mask[ref_index* block_len: (ref_index + 1) * block_len] = True
                mask = mask.to(device)
            ...
            with torch.cuda.amp.autocast(dtype=ptdtype):
                ...
                if args.use_distill:
                    distill_loss = args.distill_loss_weight * semantic_encoder.compute_distill_loss(edited_img, llm_features) # type: ignore
